...
class CilimaoSearchService : TorrentSearchService {
    ...
    override fun search(key: String, page: Int): List<TorrentInfo> {
        ...
        runCatching {
            ...
            items.forEachIndexed { _, element ->
                runCatching {
                    val title =
                        element.getElementsByClass("title")[0].getElementsByTag("h3")[0].text()
                    ...
                    val link =
                        element.getElementsByClass("title")[0].getElementsByTag("h3")[0].getElementsByAttribute(
                            "href"
                        )[0].attr("href")
                    val magnetUrl = bar.getElementsByAttribute("href").attr("href")
                    val hash = magnetUrl.replace(KEY_MAGNET_PREFIX, "").uppercase()
                    list.add(
                        TorrentInfo(
                            hash = hash,
                            title = title,
                            ...
                            magnetUrl = magnetUrl,
                            torrentUrl = hash.hashToTorrentUrl,
                            size = size,
                            date = date,
    ...
    override fun findMagnetUrl(url: String): String {
        runCatching {
            return KEY_MAGNET_PREFIX + url.substring(url.lastIndexOf("/") + 1).replace(".html", "")
                .uppercase()
        }
        return ""
    }
